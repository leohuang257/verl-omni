...
Lance's text-to-image serving path (inherited from BAGEL) uses raw
``tokenizer.encode(user_text)`` wrapped with ``<|im_start|>`` / ``<|im_end|>``
markers — no chat template.  This script pre-tokenizes prompts in that format
so the training adapter can read ``prompt_token_ids`` directly instead of
decoding + re-encoding at every step, keeping rollout and trainer token
streams identical.
...
``--model_path`` may point at the bundled Lance repo root (containing
``Lance_3B/``) or at the ``Lance_3B`` checkpoint directory itself.
...
def resolve_tokenizer_dir(model_path: str) -> str:
    """Resolve the tokenizer directory from a Lance repo root or checkpoint dir."""
    model_path = os.path.expanduser(model_path)
    if os.path.isfile(os.path.join(model_path, "tokenizer.json")):
        return model_path
    subdir = os.path.join(model_path, "Lance_3B")
    if os.path.isfile(os.path.join(subdir, "tokenizer.json")):
        return subdir
    raise FileNotFoundError(f"No Lance tokenizer found at {model_path!r} (or its Lance_3B/ subdirectory).")
...
def tokenize_lance_prompt(
    ...
    """Tokenize a user prompt in Lance's native t2i format.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path",
        required=True,
        help="Path to the Lance model directory (repo root or Lance_3B/) for the tokenizer.",
    )
    parser.add_argument(
        "--input_dir",
    ...
    tokenizer_dir = resolve_tokenizer_dir(args.model_path)
