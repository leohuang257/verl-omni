...
Extends ``LancePipeline`` with an SDE scheduler for stochastic denoising and
log-probability recording on the text-to-image path (inherited from
``BagelPipeline.forward``).  Applies per-GPU SDE windowing so noise is only
injected on a contiguous subset of denoising steps, and injects Lance's
sampling defaults (``timestep_shift=3.5``) which the inherited t2i path would
otherwise replace with BAGEL's.
"""
...
# The pinned LancePipeline t2i path reads prompt text from
# ``req.prompts[0]["prompt"]`` only; token ids from the agent loop are decoded
# back to the raw caption (Lance t2i uses ``[<|im_start|>] caption
# [<|im_end|>]`` — no chat template on the serving side).
_CHAT_MARKERS = (
    "<|vision_start|>",
    "<|vision_end|>",
    "<|image_pad|>",
...
class _AdapterStepOutput:
    """Adapter output matching what the denoising loop expects from ``scheduler.step``."""

    prev_sample: torch.Tensor
    log_prob: torch.Tensor | None


class _LanceSchedulerAdapter:
    """Adapt ``FlowMatchSDEDiscreteScheduler`` to the Lance/BAGEL calling convention.

    The denoise loop calls ``scheduler.step(v_t, sigma, x_t, dt, **kwargs)``
    with 4 positional args; the diffusers scheduler expects 3.  SDE noise and
    log-prob recording are gated to a per-request window so steps outside the
    window run deterministically (ODE, ``noise_level=0``).
    """
    ...
    def begin_forward(
        ...
        Args:
            sde_window: ``(begin, end_exclusive)`` step range where SDE
                ...
                disables windowing (noise at every step).
    ...
    def step(
        ...
        **kwargs,
        ...
        Args:
            ...
            sigma: Current noise level (raw shifted sigma, not 0-1000).
...
def _pick_sde_window(
    ...
    Uses ``seed`` directly so that all rollouts on one GPU share the same SDE
    window, matching the official flow_grpo behaviour of
...
class LancePipelineWithLogProb(LancePipeline):
    """Lance pipeline variant for RL rollouts with verl-omni."""

    def __init__(self, *, od_config: OmniDiffusionConfig, prefix: str = ""):
        super().__init__(od_config=od_config, prefix=prefix)
        inner = FlowMatchSDEDiscreteScheduler()
        self.scheduler = _LanceSchedulerAdapter(inner)
        logger.info("LancePipelineWithLogProb: SDE scheduler enabled for RL rollouts")
    ...
    def _decode_token_prompt(self, token_ids: Any) -> str | None:
        """Decode Lance token IDs to a cleaned prompt text string."""
    ...
    def _ensure_lance_prompt_text(self, req: OmniDiffusionRequest) -> None:
    ...
    def forward(self, req: OmniDiffusionRequest) -> DiffusionOutput:
        self._ensure_lance_prompt_text(req)
        ...
        # Apply Lance sampling + CFG defaults so rollout and training log-prob
        # recomputation match.  The inherited t2i path defaults
        # ``timestep_shift`` to BAGEL's 3.0 — inject Lance's 3.5 explicitly.
        extra_args.setdefault("timestep_shift", LANCE_TIMESTEP_SHIFT)
        for k, v in LANCE_FLOWGRPO_CFG_DEFAULTS.items():
        ...
        # FlowGRPO compares quadratic log-prob terms only.
        self.scheduler_kwargs["include_logprob_normalizer"] = False

        # Per-request scheduler setup matching the training-side sigma schedule.
        assert req.sampling_params.num_inference_steps is not None, "num_inference_steps must be set for RL rollouts"
        lance_num_timesteps = int(req.sampling_params.num_inference_steps)
        setup_lance_sigmas(
            self.scheduler._inner,
            lance_num_timesteps,
            shift=float(extra_args["timestep_shift"]),
        )
        ...
        # vllm-omni 0.22 runs one extra denoise step; compensate for schedule parity.
        req.sampling_params.num_inference_steps = vllm_omni_num_timesteps(lance_num_timesteps)
        try:
            output = super().forward(req)
        finally:
            req.sampling_params.num_inference_steps = lance_num_timesteps
